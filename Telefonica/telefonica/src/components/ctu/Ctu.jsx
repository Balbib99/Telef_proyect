...
export const Ctu = () => {
  ...
  const handleButtonClick = async (buttonName) => {
    ...
    if (buttonName === "submitBtn") {
      ...
      // Peticion al backend
      const request = await fetch(Global.url + "ctu/save", {
        method: "POST",
      ...
      // Peticion al backend
      const request = await fetch(Global.url + "ctu/search", {
        method: "POST",
      ...
      if (data.status == "success") {
        ...
        descriptionRef.current.value = data.ctu.description;
  ...
  return (
    <>
      ...
      {saved == "sended" ?
        <strong className='alert alert-success'> CTU guardado correctamente !! </strong>
        : ""}
